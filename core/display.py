...
def init_os():
    ...
    try:
        ...
        print("error")
        pass
    System.Title(f"*Larpkido @ {user} ~ v{ver}")
...
def get_inpt(prompt=None):
    if prompt is None:
        prompt = f"{user}@larpkido:~#"
    else:
        prompt = prompt.replace("larpkido@root/", f"{user}@larpkido/")
        prompt = prompt.replace("larpkido@", f"{user}@larpkido/")
